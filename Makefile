...
YYYY := $(shell echo "$$SQLITE_DOWNLOAD_PAGE" | grep -oP 'href="\K\d+(?=/sqlite-amalgamation-\d+\.zip)' | head -n1)
SQLITE_AMALGATION := $(shell echo "$$SQLITE_DOWNLOAD_PAGE" | grep -oP 'href="[^"]*sqlite-amalgamation-\d+\.zip"' | sed -n 's/.*\/\(sqlite-amalgamation-[0-9]*\)\.zip.*/\1/p')
